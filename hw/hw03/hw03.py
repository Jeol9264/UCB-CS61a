...
def pingpong(n):
    ...
    return ans
    '''
    # recusive methods(required)
    def ans(pos, neg, res):
        if pos == n:
            return res
        elif pos % 8 == 0 or num_eights(pos) > 0:
            return ans(pos+1, -neg, res-neg)
        else:
            return ans(pos+1, neg, res+neg)
    return ans(1, 1, 1)
...
def count_coins(change):
    ...
    """
    "*** YOUR CODE HERE ***"
    def count_coins2(change, min_coin):
        if change == 0:
            return 1
        elif not min_coin:
            return 0
        elif change < min_coin:
            return 0
        else:
            second_min_coin = ascending_coin(min_coin)
            return count_coins2(change, second_min_coin) + count_coins2(change-min_coin, min_coin)
    return count_coins2(change, 1)
