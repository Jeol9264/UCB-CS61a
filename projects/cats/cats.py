...
def choose(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    selected_pargraphs = [paragraph for paragraph in paragraphs if select(paragraph)]
    if k >= len(selected_pargraphs):
        return ''
    else:
        return selected_pargraphs[k]
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    def hastopic(paragraph):
        paragraph_wordlist = split(paragraph)
        paragraph_wordlist = list(map(remove_punctuation, list(map(lower, paragraph_wordlist))))
        for word in paragraph_wordlist:
            if word in topic:
                return True
...
def accuracy(typed, reference):
    ...
    if type_len + reference_len == 0:
        return 100.
    elif type_len * reference_len == 0:
        return 0.
    else:
        count = 0
        for i in range(min(type_len, reference_len)):
...
def wpm(typed, elapsed):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    word_length = len(typed)
    return word_length * 12. / elapsed
    # END PROBLEM 4
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    "*** YOUR CODE HERE ***"
    """
    1. typed_word in valid_words -> typed word
    2. typed_word not in valid_words
        1. compute diff_func
    """
    ...
    proba_word = min(valid_words, key = diff_typed)
    diff = diff_typed(proba_word)
    if diff > limit:
        return typed_word
    else:
        return proba_word
...
def fastest_words(match):
    ...
    for word in word_indices:
        fastest_player, min_time = min(zip(player_indices, [player[word] for player in get_times(match)]), key=lambda item: item[1])
        ans[fastest_player].append(get_words(match)[word])
