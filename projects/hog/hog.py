...
def picky_piggy(score):
    ...
    """
    # assert 0 <= score < GOAL_SCORE, 'score is not valid'  # 测试时不需要
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    digit = int(((10 ** (score % 6 + 6) / 7) % 10) // 1)
    return digit
    # END PROBLEM 2
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            ...
            bonus = hog_pile(score0, score1)
            score0 += bonus
        else:
            num_rolls = strategy1(score1, score0)
            score1 += take_turn(num_rolls, score0, dice, goal)
            bonus = hog_pile(score1, score0)
            score1 += bonus
        who = next_player(who)
    # END PROBLEM 5
    ...
    # BEGIN PROBLEM 6
        "*** YOUR CODE HERE ***"
        if score1 == 0:
            f_comment = say(score0, score1)
        else:
            f_comment = f_comment(score0, score1)
    # END PROBLEM 6
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    #         running_high = gain
    #         print(gain, 'point(s)! That\'s a record gain for Player %d!'% who)
    ...
    def say(score0, score1):
        ...
        if gain > running_high:
            print(gain, 'point(s)! That\'s a record gain for Player %d!'% who)
            return announce_highest(who, new_score, gain)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    average_max = 0
    while x <= 10:
        if (cur_dice := averaged_dice(x, dice)) > average_max:
            x_max = x
            average_max = cur_dice
...
def picky_piggy_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    if (pp_score := picky_piggy(opponent_score)) >= cutoff:
        return 0
    else:
        return num_rolls
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if (pp_score := picky_piggy(opponent_score)) == opponent_score - score:
        return 0
    else:
